...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { image_base64, mode = "outfit", user_id } = body;
    ...
    if (!user_id) {
      return jsonError("Missing user_id field", 400);
    }
    ...
    try {
      ...
      console.log(`Vision pipeline returned ${visionData.items_found} items`);
